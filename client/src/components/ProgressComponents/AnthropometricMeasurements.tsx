...
const AnthropometricMeasurements = (data: any) => {
  const historicalData = data.historicalData;

  const { saveMeasurement } = useMeasurementPeriod();
  const [measurements, setMeasurements] = useState({
    weight: { current: 70, unit: 'kg' },
    height: { current: 175, unit: 'cm' },
    waist: { current: 80, unit: 'cm' },
    hip: { current: 95, unit: 'cm' },
    bodyFatMass: { current: 14, unit: 'kg' },
    leanBodyMass: { current: 56, unit: 'kg' }
  });
  ...
  const handleSaveMeasurement = (type, value) => {
    // Save to local state
    setMeasurements(prev => {
      ...
      calculateDerivedMeasurements(updated);
      
      return updated;
    });
    
    // Save to the period-tracking system
    saveMeasurement(type, value);
  ...
  const calculateDerivedMeasurements = (data) => {
    ...
    if (data.weight && data.height) {
      ...
      data.bmi = {
        current: parseFloat((data.weight.current / (heightInMeters * heightInMeters)).toFixed(1)),
        unit: 'kg/m²'
      };
    }
  ...
  const advancedMeasurementTypes = [
    { 
      ...
      tooltip: {
        ...
        description: 'Measures body fat distribution and is an indicator of health risk.',
        men: { 
          healthy: '<0.95',
          ranges: ['<0.90: Low risk', '0.90-0.95: Moderate risk', '>0.95: High risk']
        },
        women: {
          healthy: '<0.80',
          ranges: ['<0.75: Low risk', '0.75-0.80: Moderate risk', '>0.80: High risk']
        }
      }
    },
    { 
      ...
      tooltip: {
        ...
        description: 'A measure of body fat based on height and weight. Same for men and women.',
        ranges: ['<18.5: Underweight', '18.5-24.9: Normal weight', '25.0-29.9: Overweight', '30.0+: Obesity'],
    ...
    { 
      ...
      tooltip: {
        title: 'Body Fat Mass',
        description: 'The total weight of fat in your body. Healthy body fat percentage ranges:',
        men: {
          ranges: [
            'Essential fat: 2-5%',
            'Athletes: 6-13%',
    ...
    { 
      ...
      tooltip: {
        ...
        description: 'The weight of everything in your body except fat, including muscle, organs, bones, blood, etc.',
  ...
  const getMeasurementValue = (id) => {
    if (id === 'waistToHipRatio') {
      return derivedMeasurements.waistToHipRatio?.current || (measurements.waist.current / measurements.hip.current).toFixed(2);
    } else if (id === 'bmi') {
      const heightInMeters = measurements.height.current / 100;
      return derivedMeasurements.bmi?.current || (measurements.weight.current / (heightInMeters * heightInMeters)).toFixed(1);
    } else {
    ...
    <div className="anthropometric-measurements">
      ...
      >
        {activeMeasurementTypes.map((type) => (
          ...
          >
            {(type.id === 'waistToHipRatio' || type.id === 'bmi' || type.id === 'bodyFatMass' || type.id === 'leanBodyMass') && (
                ...
                <div className="relative group">
                  ...
                  <div className="absolute z-20 invisible group-hover:visible bg-white p-3 rounded-lg shadow-lg border border-gray-200 w-64 text-xs right-0 top-6">
                    {type.id === 'waistToHipRatio' && (
                      <>
                        <h5 className="font-bold mb-1 text-[#4D7051]">Waist-to-Hip Ratio</h5>
                        <p className="mb-2">Measures body fat distribution and is an indicator of health risk.</p>
                        <p className="mb-1"><span className="font-semibold">Men:</span> Healthy range: &lt;0.95</p>
                        <ul className="ml-4 mb-1">
                          <li>&lt;0.90: Low risk</li>
                          <li>0.90-0.95: Moderate risk</li>
                          <li>&gt;0.95: High risk</li>
                        </ul>
                        <p className="mb-1"><span className="font-semibold">Women:</span> Healthy range: &lt;0.80</p>
                        <ul className="ml-4">
                          <li>&lt;0.75: Low risk</li>
                    ...
                    {type.id === 'bmi' && (
                      <>
                        <h5 className="font-bold mb-1 text-[#4D7051]">Body Mass Index (BMI)</h5>
                        <p className="mb-2">A measure of body fat based on height and weight. Same for men and women.</p>
                        <ul className="ml-4">
                        ...
                        <p className="mt-1 text-gray-500">Note: BMI doesn't account for muscle mass, bone density, or overall body composition.</p>
                      </>
                    )}
                    {type.id === 'bodyFatMass' && (
                      <>
                        <h5 className="font-bold mb-1 text-[#4D7051]">Body Fat Mass</h5>
                        <p className="mb-2">The total weight of fat in your body. Healthy body fat percentage ranges:</p>
                        <p className="mb-1"><span className="font-semibold">Men:</span></p>
                        <ul className="ml-4 mb-1">
                        ...
                        <p className="mb-1"><span className="font-semibold">Women:</span></p>
                        <ul className="ml-4">
                    ...
                    {type.id === 'leanBodyMass' && (
                      <>
                        <h5 className="font-bold mb-1 text-[#4D7051]">Lean Body Mass</h5>
                        <p className="mb-2">The weight of everything in your body except fat, including muscle, organs, bones, blood, etc.</p>
                        <p className="mb-1">Typical ranges as % of total weight:</p>
                        <p className="mb-1"><span className="font-semibold">Men:</span> 75-85% of total body weight</p>
                        <p className="mb-1"><span className="font-semibold">Women:</span> 65-75% of total body weight</p>
                        <p className="mt-1">Higher lean mass generally indicates better physical fitness and metabolic health.</p>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <h4 className="text-sm font-medium text-gray-700">{type.label}</h4>
      ...
      <div className="measurement-content bg-gray-50 rounded-lg p-4 border border-gray-200">
        <div className="flex justify-between mb-4">
          <h3 className="text-lg font-medium text-gray-800">
            {selectedMeasurement
              ? `${activeMeasurementTypes.find(t => t.id === selectedMeasurement)?.label} History`
              : 'Measurement History'}
          </h3>
          <motion.button
